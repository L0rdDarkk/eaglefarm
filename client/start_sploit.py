...
def parse_args():
    parser = argparse.ArgumentParser(description='🦅 EagleFarm - Run exploit on all teams in a loop',
        ...
        for _ in once_in_a_period(POST_PERIOD):
            ...
            if flags_to_post:
                try:
                    ...
                    logging.info('🦅 {} flags posted to EagleFarm ({} in queue)'.format(
...
def main(args):
    ...
    logging.info('🦅 Connecting to EagleFarm server at {}'.format(args.server_url))
    ...
    for attack_no in once_in_a_period(args.attack_period):
        ...
        print()
        logging.info('🦅 Launching attack #{}'.format(attack_no))
